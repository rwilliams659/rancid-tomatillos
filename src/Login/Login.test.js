...
describe('Login component', () => {
  ...
  it('on successfully validated login, should call the correct functions', async () => {
    ...
    await waitFor(() => expect(mockUpdatedUserId).toHaveBeenCalledTimes(1));
    await waitFor(() => expect(mockUpdateLoginStatus).toHaveBeenCalledTimes(1));
    await waitFor(() => expect(mockGetUserRatings).toHaveBeenCalledTimes(1));
    await waitFor(() => expect(mockSetFavoriteMovies).toHaveBeenCalledTimes(1));
  });
